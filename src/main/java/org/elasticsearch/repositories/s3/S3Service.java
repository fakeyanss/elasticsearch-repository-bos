...
class S3Service implements Closeable {
    ...
    /**
     * Client settings calculated from static configuration and settings in the
     * keystore.
     */
    private volatile Map<String, S3ClientSettings> staticClientSettings = MapBuilder
            .<String, S3ClientSettings>newMapBuilder()
            .put("default", S3ClientSettings.getClientSettings(Settings.EMPTY, "default")).immutableMap();

    /**
     * Client settings derived from those in {@link #staticClientSettings} by
     * combining them with settings in the {@link RepositoryMetaData}.
     */
    private volatile Map<S3ClientSettings, Map<RepositoryMetaData, S3ClientSettings>> derivedClientSettings 
            = emptyMap();
    ...
    /**
     * Attempts to retrieve a client by its repository metadata and settings from
     * the cache. If the client does not exist it will be created.
     */
    ...
    /**
     * Either fetches {@link S3ClientSettings} for a given
     * {@link RepositoryMetaData} from cached settings or creates them by overriding
     * static client settings from {@link #staticClientSettings} with settings found
    ...
    S3ClientSettings settings(RepositoryMetaData repositoryMetaData) {
        ...
        if (staticSettings != null) {{
                final S3ClientSettings existing = derivedClientSettings.getOrDefault(staticSettings, emptyMap())
            ...
            synchronized (this) {
                final Map<RepositoryMetaData, S3ClientSettings> derivedSettings = derivedClientSettings
                        .getOrDefault(staticSettings, emptyMap());
                ...
                derivedClientSettings = MapBuilder
                        .newMapBuilder(derivedClientSettings).put(staticSettings, MapBuilder
                                .newMapBuilder(derivedSettings).put(repositoryMetaData, newSettings).immutableMap())
                        .immutableMap();
                return newSettings;
            }
        }
    ...
    AmazonS3 buildClient(final S3ClientSettings clientSettings) {
        ...
        final String endpoint = Strings.hasLength(clientSettings.endpoint) ? clientSettings.endpoint
                : Constants.S3_HOSTNAME;
        LOGGER.debug("using endpoint [{}]", endpoint);

        // If the endpoint configuration isn't set on the builder then the default
        // behaviour is to try
        // and work out what region we are in and use an appropriate endpoint - see
        // AwsClientBuilder#setRegion.
        // In contrast, directly-constructed clients use s3.amazonaws.com unless
        // otherwise instructed. We currently
        // use a directly-constructed client, and need to keep the existing behaviour to
        // avoid a breaking change,
        // so to move to using the builder we must set it explicitly to keep the
        // existing behaviour.
        //
        // We do this because directly constructing the client is deprecated (was
        // already deprecated in 1.1.223 too)
    ...
    static ClientConfiguration buildConfiguration(S3ClientSettings clientSettings) {
        ...
        if (Strings.hasText(clientSettings.proxyHost)) {
            // TODO: remove this leniency, these settings should exist together and be
            // validated
